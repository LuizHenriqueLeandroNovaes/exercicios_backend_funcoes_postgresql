...
--cadastradosna tabela `usuarios` com idade maior ou igual a 18 anos.
...
-- 6 - Faça uma query que retorna todas as categorias não nulas e a soma 
--do estoque de todos os medicamentos de cada categoria na tabela `farmacia`.
...
-- 9 - Faça uma query que retorna uma única coluna com a junção do nome 
--do medicamento e a categoria entre parenteses de todos os registros em 
--que a categoria não seja nula da tabela `farmacia`. Ex.: `Endocet (green)`.
...
--e o nome do medicamento separado por um hífen e a categoria entre parenteses de 
--todos os registros da tabela `farmacia`. Quando a categoria for nula, substituir 
--por `(sem categoria)`. Ex.: `1 - Endocet (green)` ou `1 - Endocet (sem categoria)`.
...
-- 12 - Faça uma query que retorna o nome, a idade, o email e o tempo que cada usuario 
--menor de 18 anos possui cadastrado na tabela `usuarios`. O tempo precisa ser uma 
--coluna que retorne a quantidade de anos, meses, dias, horas, minutos e segundos 
...
-- 13 - Faça uma query que retorna o nome, a idade, o email e o tempo que cada usuario, maior 
--ou igual a 60 anos, possui cadastrado na tabela `usuarios`. O tempo precisa ser uma coluna 
...
select count(id) , categoria from farmacia where categoria is not null group by categoria;
...
select idade , count(*) registros from usuarios where idade >= 18 group by idade;
